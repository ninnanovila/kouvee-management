...
class CustomerPetRecyclerViewAdapter(private val customers: MutableList<Customer>, private val petTypes: MutableList<PetType>, private val customerPets: MutableList<CustomerPet>, private val listener: (CustomerPet) -> Unit) : RecyclerView.Adapter<CustomerPetRecyclerViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.bindItem(customers, petTypes, customerPets[position], listener)
    }
    ...
    class ViewHolder (override val containerView: View): RecyclerView.ViewHolder(containerView),
        ...
        private var customertv: TextView = itemView.findViewById(R.id.customer)
        private var birthdate: TextView = itemView.findViewById(R.id.birthdate)

        fun bindItem(customer: MutableList<Customer>,petType: MutableList<PetType>, customerPet: CustomerPet, listener: (CustomerPet) -> Unit){
            for (type in petType){
            ...
            for (each in customer){
                if (each.id.equals(customerPet.id_customer)){
                    customertv.text = each.name.toString()
                }
            }
